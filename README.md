...
Open the repo in a **Codespace** (badge or **Code → Codespace** on [github.com/CastaliaInstitute/kali-mcp](https://github.com/CastaliaInstitute/kali-mcp)). The **devcontainer** is **Kali rolling**: `[.devcontainer/Dockerfile](.devcontainer/Dockerfile)` temporarily presents **Debian bookworm** in `/etc/os-release` at **image build** so the [docker-in-d](https://github.com/devcontainers/features/tree/main/src/docker-in-docker) feature can install; `**onCreateCommand**` runs `[.devcontainer/restore-kali-os-release.sh](.devcontainer/restore-kali-os-release.sh)` first, then `[scripts/install-kali-mcp-prereqs.sh](scripts/install-kali-mcp-prereqs.sh)` (Kali tools for the MCP) then `**postCreate**` does `**pip install -e .**` (HTTP server; optional `**[copilot]**` for `kali_mcp.copilot_stdio` if you add stdio from `mcp.json.example`).
