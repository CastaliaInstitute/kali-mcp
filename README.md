...
- **kali-mcp tools** honor per-tool `**timeout_sec**` and the server cap **`KALI_MCP_MAX_TIMEOUT_SEC`** (see table above). Set it in the devcontainer `**containerEnv**`, in `mcp.json` `**env**` for `kaliMcpStdio`, or in the shell when running `kali-mcp`.
...
**MCP autostart (Copilot):** the repo sets **`chat.mcp.autostart`:** `**true**`, **`chat.mcp.autoStart`:** `**newAndOutdated**` (VS Code default: (re)start workspace MCPs when the config is new or may be outdated), and **`chat.mcp.apps.enabled`:** `**true**` in `[.devcontainer/devcontainer.json](.devcontainer/devcontainer.json)` and `[.vscode/settings.json](.vscode/settings.json)` so **`kaliMcpStdio` is started for you** instead of sitting stopped and timing out. The **stdio** process is still **`kali_mcp.copilot_stdio_lite`** (fast import). If chat feels slow on first open, use **User** settings to set **`chat.mcp.autoStart`** to `**never**` and start servers only via **MCP: List Servers**. If something else fails, temporarily set **`chat.mcp.access` to `false`** in **User** settings to test without MCP. You need a **Copilot** seat, **rebuild** after a pull, and in the **browser** Codespace avoid ad blockers blocking the Copilot API.

## GitHub Codespaces (Kali) — test the same MCP as Kaliyai

Open the repo in a **Codespace** (badge or **Code → Codespace** on [github.com/CastaliaInstitute/kali-mcp](https://github.com/CastaliaInstitute/kali-mcp)). The **devcontainer** is **Kali rolling** and on create runs `[scripts/install-kali-mcp-prereqs.sh](scripts/install-kali-mcp-prereqs.sh)` (Kali tools for the MCP) then `**pip install -e .**` (HTTP server + `**stdio_lite**`). Optional: `**pip install -e '.[copilot]'**` for `**kali_mcp.copilot_stdio**` (full PyPI `mcp` SDK).
