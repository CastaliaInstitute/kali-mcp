...
**MCP autostart in the editor:** `**chat.mcp.autostart**` / `**chat.mcp.autoStart**` / `**chat.mcp.apps.enabled**` are set in `[.vscode/settings.json](.vscode/settings.json)` and `[.devcontainer/devcontainer.json](.devcontainer/devcontainer.json)` so Copilot **starts workspace MCPs** when appropriate ([MCP in VS Code](https://code.visualstudio.com/docs/copilot/reference/mcp-configuration)).
